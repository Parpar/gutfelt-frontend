...
function KundehåndteringPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      // VIGTIGT: Kalder nu kategorien "kundehåndtering"
      // Du skal tilføje FOLDER_ID_KUNDEHANDTERING på Render og i server.js
      const response = await fetch('https://gutfelt-backend.onrender.com/api/upload/kundehåndtering', {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="widget" style={{ margin: '2rem' }}>
      ...
      <p>Best practice for kommunikation med kunder og vedligeholdelse af kunderelationer.</p>

      <ul className="document-list">
        {documents.map(doc => (
          <li key={doc.id} className="document-item">
            {/* Ikonet kan ændres til noget mere passende, f.eks. fa-headset */}
            <div className="document-icon"><i className="fas fa-headset"></i></div> 
      ...
      {/* For nu kan HR-redaktøren uploade alt. Dette kan ændres til en anden rolle senere. */}
